...
SELECT product_id, count(product_id) FROM assignment.Sales
group by product_id;

-- 11. Write a query to find the total quantity sold for each product from the `Sales` table.
---SELECT * FROM assignment.Sales;
select product_id, sum(quantity_sold) from assignment.Sales
group by product_id;
...
select * from assignment.customers 
where customer_id in (select customer_id from assignment.sales
...
select c.first_name, c.last_name, s.total_amount from assignment.sales s
...
select concat(c1.first_name, ' & ', c2.first_name) as name_pair, c1.membership_status as status from assignment.customers c1
...
select p.product_name, count(sale_id) as total_sales
from assignment.sales s
join assignment.products p
...
select c.first_name, c.customer_id from 
...
order by c.first_name, c.customer_id;
...
select p.product_name, sum(p.price * s.quantity_sold)
...
select distinct c.first_name, c.last_name, s.sale_date from 
assignment.customers c
join assignment.sales s
on s.customer_id = c.customer_id
...
with sale_2023 as (
select c.first_name, c.last_name, s.total_amount from 
assignment.customers c
join assignment.sales s
on s.customer_id = c.customer_id
where s.sale_date >= '2023-01-01' and s.sale_date < '2024-01-01'
order by  s.total_amount desc)
select  *  from sale_2023
limit 1;
...
select p.product_name, s.total_amount as expen
from assignment.products p
join assignment.sales s
on s.product_id = p.product_id
order by expen desc
...
with total_customer as (select c.first_name, s.total_amount
from assignment.customers c
join assignment.sales s
on s.customer_id = c.customer_id
...
with cumlative as (select  c.first_name, count(p.product_id), sum(s.quantity_sold) as tot  from 
assignment.customers c
join assignment.sales s
on s.customer_id = c.customer_id
join assignment.products p
...
select * from cumlative 
where tot > 5;

-- 30. Write a query to find the average quantity sold per product.

select p.product_name, avg(s.quantity_sold)  av
from assignment.products p
join assignment.sales s
on s.product_id = p.product_id
group by p.product_name;
...
select c.first_name, s.customer_id, sum(s.total_amount)
from assignment.customers c
join assignment.sales s
on s.customer_id = c.customer_id
where s.sale_date >= '2023-01-01' and s.sale_date < '2024-01-01'
group by c.first_name, s.customer_id
...
select p.product_name, p.product_id, s.sale_id,  sum(s.total_amount) AS total_sales
from assignment.products p
join assignment.sales s      
on s.product_id = p.product_id
group by p.product_name, p.product_id, s.sale_id
order by s.total_amount desc;
...
select distinct c.first_name, s.customer_id
from assignment.customers c
join assignment.sales s
on s.customer_id = c.customer_id
...
SELECT c.customer_id, c.first_name, count(*) as pch
from assignment.customers c
join assignment.sales s
on c.customer_id = s.customer_id
...
select  c.customer_id, c.first_name, sum(s.quantity_sold) as tot_qnt  from 
assignment.customers c
join assignment.sales s
on s.customer_id = c.customer_id
group by c.customer_id, c.first_name;
...
select product_name, stock_quantity from assignment.products where stock_quantity = (select stock_quantity from assignment.products
...
--select product_name from assignment.products where product_name like '%phone%';
...
select  p.product_name, sum(s.total_amount) as tot from 
assignment.customers c
join assignment.sales s
on s.customer_id = c.customer_id
join assignment.products p
...
select p.product_name, sum(s.total_amount), to_char(s.sale_date, 'YYYY-MM' )
from assignment.products p
join assignment.sales s      
on s.product_id = p.product_id
...
select s.product_id from assignment.sales s
join assignment.inventory  i
...
select  c.first_name, c.customer_id, s.total_amount   from 
assignment.customers c
join assignment.sales s
on s.customer_id = c.customer_id
order by s.total_amount desc
...
select distinct p.product_id, count(*)
from assignment.products p
join assignment.sales s      
on s.product_id = p.product_id
...
select p.product_id, p.product_name, p.price, sum(s.quantity_sold)  total_sold
from assignment.products p
join assignment.sales s 
on p.product_id = s.product_id
where p.price > 200
